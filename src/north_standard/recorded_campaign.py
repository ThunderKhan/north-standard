...
def campaign_summary(rows: list[RecordedTrialResult]) -> dict[str, Any]:
    ...
    compliant_inconclusive = sum(
        row.decision == Decision.INCONCLUSIVE for row in compliant
    )
    ...
    for condition in sorted({row.condition for row in rows}):
        ...
        per_condition[condition] = {
            ...
            "ground_truth_counts": dict(
                Counter(row.ground_truth.value for row in selected)
            ),
    ...
    return {
        ...
        "inconclusive_rate_compliant": _interval(
            compliant_inconclusive, len(compliant)
        ),
